...
from .image_handler import make_dataset_image_synth_train, make_dataset_image_synth_test, make_dataset_image_poled,make_dataset_image_toled
...
class UDCDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if opt.dataset == "SYNTH":
            if opt.phase == 'train':
                self.imgX_paths, self.imgY_paths = make_dataset_image_synth_train(self.dir_img)
            else:
                self.imgX_paths, self.imgY_paths = make_dataset_image_synth_test(self.dir_img)
        elif opt.dataset == "TOLED":
            self.imgX_paths, self.imgY_paths = make_dataset_image_toled(self.dir_img)
        else:
            self.imgX_paths, self.imgY_paths = make_dataset_image_poled(self.dir_img)
        
        if opt.phase == 'train':
            if opt.dataset == "SYNTH":
                self.transform_list = [custom_transforms.ToTensor()]
                # self.transform_list = [custom_transforms.RandomVerticalFlip2(),
                #                        custom_transforms.RandomHorizontalFlip2(),
                ...
                self.transform_list = [custom_transforms.RandomVerticalFlip(),
                                       ...
                                       custom_transforms.RandomCrop((512, 512)),
                                       custom_transforms.ToTensor()
                                       ]
        else:
            if opt.dataset == "SYNTH":
