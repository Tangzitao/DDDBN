...
from .UDC_DDDBNet import UDCDDDBnet
...
class Model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if not self.isTrain or opt.continue_train or opt.phase == 'test':
            self.load_network(self.net, 'udc', opt.which_epoch)
    ...
    def forward(self):
        ...
        # self.x_high, self.x_low, self.x_hat1, self.x_hat2, self.x_hat = self.net(self.Y_rgb)

    def backward(self):
        self.loss_recon = self.content_loss.get_loss(self.x_hat, self.X_rgb)
        if self. opt.dataset=='SYNTH':
            self.loss_perceptual = self.perceptual_loss.get_loss(self.x_hat, self.X_rgb)
            self.loss_total = self.loss_recon + 0.01* self.loss_perceptual 
        else:
            self.loss_perceptual = self.perceptual_loss.get_loss(self.x_hat, self.X_rgb)
            self.loss_total = self.loss_recon + 0.05* self.loss_perceptual
    ...
    def get_current_errors(self):
        if self. opt.dataset=='SYNTH':
            return OrderedDict([('loss_recon', self.loss_recon.data.cpu().numpy()),
                            ('loss_perceptual', self.loss_perceptual.data.cpu().numpy()*0.01),
    ...
    def save(self, label):
        self.save_network(self.net, 'udc', label, self.gpu_ids)
